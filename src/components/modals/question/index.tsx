...
const QuestionModal = ({ visible, confirm, iconConfirm, iconReject, reject, title, paragraph }: IQuestionModal) => {
  return (
    <div
      className={`fixed w-full px-6 h-screen flex justify-center left-0 items-center top-0 duration-300 z-[2000] ${visible ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
    >
      ...
      <div
        className={`bg-[#1A1C1D] w-full max-w-[426px] z-20 flex flex-col px-6 py-6 justify-center items-center gap-4 rounded-xl ${visible ? 'animate-fadeOut' : 'animate-fadeIn'}`}
      >
