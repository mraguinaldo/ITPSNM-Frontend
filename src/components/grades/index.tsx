...
const Grades = () => {
  return (
    <section className="pt-40 lg:px-6 bg-white lg:bg-transparent">
              ...
              <tr className="bg-[#F8C40D]">
                {STUDENT.grades.THEAD.map(({ content, id, visible, colSpan }) => (
                  <th
                    key={id}
                    colSpan={colSpan}
                    className={`border-[2px] border-[#D9D9D9] text-[14px] sm:text-[16px] text-center py-2 ${visible ? 'opacity-1' : 'opacity-0'}`}
                  >
                    {content}
                  </th>
                ))}
              ...
              {STUDENT.grades.TBODY.map(({ id, values }) => (
                <tr key={id}>
                  {values.map(({ id, value, visible }) => (
                    <td
                      key={id}
                      className={`border-[2px] text-[14px] sm:text-[16px] border-[#D9D9D9] w-[60px] text-center py-2
                      ...
                      ${
                        typeof value === 'number'
                          ? value < 10
                            ? 'text-[#E70F0F]'
                            : 'text-[#3757FF]'
                          : 'text-[#000] text-start pl-4 w-[240px]'
                      }`}
                    >
                      {typeof value === 'string'
                        ? value.length > 20
